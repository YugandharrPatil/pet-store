...
import { boolean, foreignKey, integer, jsonb, numeric, pgEnum, pgSequence, pgTable, serial, text, timestamp, uuid } from "drizzle-orm/pg-core";
...
export const petAddresses = pgTable(
	"pet_addresses",
	{
		id: uuid().defaultRandom().primaryKey().notNull(),
...
export const petOrders = pgTable(
	"pet_orders",
	{
		id: uuid().defaultRandom().primaryKey().notNull(),
		userId: uuid("user_id").notNull(),
...
export const petOrderItems = pgTable(
	"pet_order_items",
	{
		id: uuid().defaultRandom().primaryKey().notNull(),
