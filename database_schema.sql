...
ALTER TABLE public.pet_users ENABLE ROW LEVEL SECURITY;
...
CREATE TABLE public.pet_products (
  ...
  pet_type text CHECK (pet_type IN ('dog', 'cat')) NOT NULL,
  age_group text CHECK (age_group IN ('puppy', 'kitten', 'adult', 'senior')) NOT NULL,
  ingredients text,
  feeding_guide text,
  image_urls text[],
  created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL
...
ALTER TABLE public.pet_addresses ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.pet_order_items ENABLE ROW LEVEL SECURITY;
